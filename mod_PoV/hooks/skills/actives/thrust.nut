::TLW.HooksMod.hook("scripts/skills/actives/thrust", function ( q ) {
	...
	q.setItem = @(__original) function(_item)
	{
		...
		else if (this.m.IsSword)
		{
			...
			if (_item.getID() == "weapon.pov_vattghern_longsword" || _item.getID() == "weapon.pov_vattghern_sword")
			{
	...
	q.isUsable = @(__original) function()
	{
		...
		if (item != null && item.isItemType(this.Const.Items.ItemType.RangedWeapon) && this.m.IsGun == false)
		{
